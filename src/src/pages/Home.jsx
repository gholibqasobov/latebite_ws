...
const whyCards = [
  { icon: Percent, title: 'Скидки 50–70%', text: 'Любимые блюда из кафе и ресторанов по цене в 2–3 раза ниже.' },
  { icon: MapPin, title: 'Еда рядом с вами', text: 'Находите предложения поблизости на интерактивной карте Алматы.' },
  { icon: Leaf, title: 'Меньше отходов', text: 'Каждая спасённая порция сокращает выбросы примерно на 2,5 кг CO₂.' },
]

const steps = [
  { icon: Search, title: 'Найдите предложение', text: 'Откройте карту и выберите блюдо в кафе рядом.' },
  { icon: CreditCard, title: 'Оплатите через Kaspi', text: 'Забронируйте и оплатите в пару касаний.' },
  { icon: ShoppingBag, title: 'Заберите заказ', text: 'Покажите QR-код в кафе и заберите еду.' },
]
    ...
    <div>
      ...
      <section className="section pt-10 pb-12 lg:pt-16 lg:pb-20 grid lg:grid-cols-2 gap-10 items-center">
        <div className="animate-fade-up">
          <span className="chip bg-mint/50 text-primary font-semibold">🌱 Спасаем еду в Алматы</span>
          <h1 className="mt-4 text-4xl sm:text-5xl lg:text-6xl font-extrabold text-primary leading-[1.05]">
            СПАСАЙ ЕДУ.<br />ЭКОНОМЬ ДЕНЬГИ.<br />
            <span className="text-accent drop-shadow-sm">БЕРЕГИ ПЛАНЕТУ.</span>
          </h1>
          <p className="mt-5 text-lg text-primary-dark/70 max-w-md">
            LateBite соединяет вас с кафе и ресторанами, у которых остались
            нераспроданные блюда. Берите вкусную еду со скидкой до 70% — и
            помогайте сократить пищевые отходы.
          </p>
          <div className="mt-7 flex flex-wrap gap-3">
            <Link to="/offers" className="btn-primary text-base !px-7 !py-3">
              Смотреть предложения <ArrowRight size={18} />
            </Link>
        ...
        <div className="relative animate-fade-up">
          ...
          <div className="absolute -bottom-5 -left-3 sm:left-6 card px-4 py-3 flex items-center gap-3">
            ...
            <div>
              <div className="text-xs text-primary-dark/60">Уже спасено</div>
              <div className="font-extrabold text-primary">128 450 кг CO₂</div>
            </div>
          </div>
        </div>
      </section>
      ...
      <section className="section py-12">
        <h2 className="text-center text-3xl font-extrabold text-primary">Почему LateBite</h2>
      ...
      <section className="bg-white py-14">
        <div className="section">
          <h2 className="text-center text-3xl font-extrabold text-primary">Как это работает</h2>
          <div className="mt-10 grid gap-8 md:grid-cols-3 relative">
            {steps.map((s, i) => (
              <div key={s.title} className="text-center relative">
                ...
                <div className="mt-2 text-accent font-extrabold text-sm">Шаг {i + 1}</div>
      ...
      <section className="section py-16">
        <div className="rounded-[2rem] bg-primary text-white p-8 sm:p-12 text-center relative overflow-hidden">
          ...
          <h2 className="text-3xl font-extrabold relative">Наш общий вклад</h2>
          <p className="mt-2 text-white/70 relative">Вместе сообщество LateBite уже изменило ситуацию</p>

          <div className="mt-10 grid gap-8 sm:grid-cols-3 relative">
            <EcoStat icon={Cloud} value={globalEco.co2} unit="кг CO₂" label="сэкономлено выбросов" />
            <EcoStat icon={Utensils} value={globalEco.meals} unit="порций" label="спасено от мусора" />
            <EcoStat icon={TreePine} value={globalEco.trees} unit="деревьев" label="условно сохранено" />
          </div>
        </div>
      </section>
    </div>
